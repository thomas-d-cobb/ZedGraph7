...
*/
		...
		virtual internal string MakeLabel( GraphPane pane, int index, double dVal )
		{
            ...
            // (YW Oct 2011) above code is modified as below to support "isUseTenPower" in linear scale
            if (_isUseTenPower)
            {
                if (SafeLog(dVal) >= 15)
                {
                    return string.Format("{0:0.######}", dVal/1000000000000000 + " (10^15)");
                }
                else if (SafeLog(dVal) >= 12)
                {
                    return string.Format("{0:0.######}", dVal/1000000000000 + " tn");
                }
                else if (SafeLog(dVal) >= 9)
                {
                    return string.Format("{0:0.######}", dVal/1000000000 + " bn");
                }
                else if (SafeLog(dVal) >= 6)
                {
                    return string.Format("{0:0.######}", dVal/1000000 + " mn");
                }
                else
                {
...
//				float lastPixVal = -10000;
						...
						float shift, float maxSpace, float scaledTic, float charHeight, float scaleFactor )
		{
            ...
            // (YW Oct 2011) code is commented as below to support "isUseTenPower" in log scale
            //if ( this.IsLog && _isUseTenPower )
